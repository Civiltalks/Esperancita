...
2. **Proponho** — "Quer que eu crie uma skill pra isso?"
3. **Se aprovado** — Crio o `SKILL.md` na categoria certa, atualizo registry, confirmo
...
2. **Aula B1 do curso (~semana 2):** 30-50 linhas. Adiciona "como penso", regras de prioridade.
3. **Operação (~mês 2-3):** 100+ linhas. Adiciona padrões descobertos no uso real.
4. **Maduro (~mês 6+):** estabiliza.
